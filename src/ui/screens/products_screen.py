...
class ProductsScreen(ctk.CTkFrame):
    ...
    def _refresh_table(self, products=None):
        ...
        for row, product in enumerate(products, start=1):
            ...
            # Quantity with color coding (simple example)
            ...
            ctk.CTkButton(action_frame, text="✏️ Edit", width=35, height=35, corner_radius=8,
                          fg_color="#3498DB", hover_color="#2980B9", 
                          command=lambda p=product: self._edit_product(p)).pack(side="left", padx=5)
            ctk.CTkButton(action_frame, text="🗑️ Delete", width=35, height=35, corner_radius=8,
    ...
    def _add_product_dialog(self):
        # Sample add logic
        pid = f"P{len(self.app.product_manager.products) + 1:03d}"
        self.app.product_manager.add_product(pid, "New Premium Item", "Electronics", "BrandX", 299.99, 25)
        self._refresh_table()

    def _edit_product(self, product):
        print(f"Edit requested for: {product.name}")
